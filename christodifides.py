...
def bee_path(bee_data, ax):
    ...
    approx_tour = christofides(G)
 
    prev_length = float('inf')
    
    while True:
        # Apply 2-opt to find a better route
        hamiltonian_circuit_optimized = two_opt(approx_tour, G)
        
        # Calculate the length of the optimized route
        current_length = sum(calculate_distance(G.nodes[hamiltonian_circuit_optimized[k]]['pos'], G.nodes[hamiltonian_circuit_optimized[k - 1]]['pos']) for k in range(1, len(hamiltonian_circuit_optimized)))
        
        # If the length didn't improve, break out of the loop
        if current_length >= prev_length:
            break

        # Update the previous length and the best-known Hamiltonian circuit
        prev_length = current_length
        approx_tour = hamiltonian_circuit_optimized
